...
#set directory
setwd("C:/Users/jormar/Dropbox/Primates_IVSO project/Statistical methods and analyses")
...
setwd("C:/Users/jormar/Dropbox/Primate_IVSO files2")

#generate 10 random datasets with random sampling of SO for species
#with multiple primary SO
df_10 = lapply(1:10, function(x) dataGR)
MainSO_10 = lapply(df_10, FUN = function(x) { 
  s = sample(c(1,2), nrow(dataGR), replace = TRUE, prob = c(0.5,0.5));
  ifelse(s==1, x$Main1, x$Main2) } )
MainSO_10 = lapply(MainSO_10, #set as factor with solitary as base category
                   FUN = function(x) {xf = factor(x, levels = c("solitary", "FF", "MM", "MF", 
                                                                "MFF", "FMM", "FFMM")) })
df_10 = Map(cbind, df_10, MainSO = MainSO_10)
...
png("phyloivso_radial_tips.png", width = 5, height = 5, units = "in", res = 600)
plot.phylo(c_tree, type = "radial", show.tip.label = TRUE, edge.width = 2, cex = 0.3)
dev.off()
...
#with contour
library(phytools)
temp = aggregate(IVSO_prop ~ Genus_species, data =  df_10[[1]], FUN = mean)
x = temp[,2]
names(x) = temp[,1]
...
#with contour and tip labels
library(phytools)
temp = aggregate(IVSO_prop ~ Genus_species, data =  df_10[[1]], FUN = mean)
x = temp[,2]
names(x) = temp[,1]

png("phyloivso_radial_cont_tips.png", width = 10, height = 10, units = "in", res = 600)
obj = contMap(phytools::force.ultrametric(c_tree, method="extend"),
              x, plot=FALSE, type ="fan", lwd=0.5)
n = length(obj$cols)
colfunc = colorRampPalette(c("purple", "red"))
obj$cols[1:n]<-colfunc(n)
plot(obj, type ="fan", lwd = 3, spread.labels = TRUE)        
dev.off()
...
#get total counts of primary SO
table(df1$Main1)
table(df1$Main2)
...
##############################################################
#Phylogenetic predictions for superfamilies (no fixed effects)
##############################################################

#model formula
SO_m = bf(SO_counts | trials(SO_tot) ~ 
            1 + (1|superfamily) + (1|gr(phylo, cov = A)) + (1|Genus_species) + (1|obs) )
...
#estimate model
m.superf = brm(formula = SO_m + IVSO_m + set_rescor(FALSE), 
...
#and then edited in Stan to allow for estimating
...
#and then creates a plot for the effect
